...
namespace sfft
{
    ...
    struct FixedComplex
    {
        ...
        friend constexpr T real(const FixedComplex<T> &c) { return c._real; }
        friend constexpr T imag(const FixedComplex<T> &c) { return c._imag; }
         ...
         */
        friend constexpr FixedComplex scale_in_half(const FixedComplex &value)
        {
            // Use the shift operator we defined in FixedPoint
            return FixedComplex(
                T((value._real.raw + 1) >> 1),
                T((value._imag.raw + 1) >> 1));
        }
    };
